import { addWindow, bringWindowToTop, markWindowAsClosing, minimizeWindow, removeWindow, setWindowPosition, setWindowSize, setWindowSizingMode, unMinimizeWindow } from '../core/context/WindowSlice.ts'
...
type WindowActionMap =
    | { type: 'ADD_WINDOW', processId: IProcess["id"], title: IPackage['name'], icon: IPackage['iconUrl'] }
    ...
    | { type: 'SET_WINDOW_SIZING_MODE', windowId: IWindow['id'], sizingMode: IWindow['sizingMode'] }
    | { type: 'SET_WINDOW_POSITION', windowId: IWindow['id'], position: IWindow['position'] }

function useWindowManager() {

    const dispatch = useDispatch<AppDispatch>()

    const executeWindowAction = useCallback((action: WindowActionMap) => {
        switch (action.type) {
            case "ADD_WINDOW": {
